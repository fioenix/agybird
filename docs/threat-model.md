...
| Unapproved permission grant | A calling agent passes `--grant` for a rule the user never approved, widening what the next run may do | Refuse `--grant` when the workspace has no recorded session and when it accompanies `--new-session`, restrict rules to the allow-rule kinds `agy` recognizes, require the target to be an escaped literal rather than a regular expression so no spelling of a wildcard is accepted, refuse targets holding no letter or digit, journal the undo before applying a `once` grant so a killed run is repaired by the next one rather than leaving the rule on disk, and default `--grant-scope` to `once` so an unapproved rule cannot outlive one run | Each run is a separate process holding no state, so the runner cannot prove a rule came from a request it previously reported; the caller remains responsible for asking the user |
