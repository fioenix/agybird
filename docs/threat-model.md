...
| Artifact path spoofing | A provider claims an unrelated local file as generated output | Resolve canonical paths, require containment in the working directory, supported raster extensions, nonzero size, and a completed image tool event | File content can still be visually wrong or maliciously crafted |
