...
The runner also resolves an Antigravity project for the workspace and passes `--project`, because a conversation is bound to its project when it is created. With `--grant` it writes user-approved allow-rules into that project file under `~/.gemini/config/projects/`. This is the only file Agybird writes outside the workspace, and it never writes inside the repository.
